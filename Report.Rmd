...
```{r}
nn <- nnet(N ~ Beat + HourType, newdat, size=8, decay=0.1, maxit=1000)

nb <- naiveBayes(N ~ Beat + HourType, newdat)

s <- svm(N ~ Beat + HourType, newdat)

mars <- earth(N ~ Beat + HourType, newdat)

m <- randomForest(N ~ Beat + HourType,newdat)
```
