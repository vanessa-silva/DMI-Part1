...
And we take as target variable the variable N of the data used for the prediction (`modelDat`) as it represents the number of events/offenses in a beat and period day specifics, since this variable is numerical we are faced with a regression problem. 
...
####Bootstrap Method and Performance estimatio
...
As we can see, our predictive analysis involved the analysis of four predictive models, one based on neural networks (`nnet`), one on Support Vector Machines (`svm`), another on Multivariate Adaptive Regression Splines (`earth`) and other tree-based (`rpartXse`).

Initially we performed a test involving the Holdout method that consists on randomly dividing the available data sample in two sub-sets, one used for training the model and the other for testing/evaluating it. The used proportion for training was 70% and for testing was 30%.

For each one the confusion matrix was made, the summary of the respective model was analyzed, the error rate of the matrix was calculated and, with the help of the function `regr.eval`, a series of regression evaluation statistics using the true target variable values and the predicted target variable values.
Analisando e comparando cada um dos valores obtido, concluímos que os modelos mencionados (tree-based model and MARS) são os que apresentam melhores resultados, ou seja, apresentam taxas de erros das matrizes mais baixas que os outros modelos, sendo ainda que o tree-based model tem menor taxa de erro que o MARS, e as egression evaluation statistics (mean absolute error, mean squared error, root mean squared error e mean average percentage error) também são melhores nos modelos mencionados.

In a second phase, we performed a test involving the Bootstrap method that consists of training models on a random sample with replacement from the original data set and test multiple times the models on the cases that were not used on the training set and calculate the average of the scores on these repetitions (Bootstrap estimate).
We checked the summary of the different results obtained with the help of the `performanceEstimation` function that performs comparative experiments of the different models, we obtain a plot with cross validation performance estimation results.
