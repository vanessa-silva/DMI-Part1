...
Based on Hounton Police beat map ![](hpd_beat_map.png) we can reconstruct manually the `Beat` cells using the street name and block range.
...
nrow(dat[dat$Type == "-",])
```
To solve it, we replaced the invalid cells for the central value of the `Type` column:
```{r}
...
nrow(dat[dat$Suffix == "-",])
```
To solve it, we replaced the invalid cells for the central value of the `Suffix` column:
```{r}
incomplete <- is.na(dat$Suffix)
dat$Suffix[incomplete] <- centralValue(dat$Suffix)
```
